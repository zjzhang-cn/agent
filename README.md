...
- 文件上传：支持将本地文件上传到 OpenAI Files API，并在后续对话中附带 file_id 引用信息。
...
- `OPENAI_MAX_TOOL_CALL_ROUNDS`：单轮对话中最大工具调用次数，防止无限循环。超出后终止并返回结果。默认值为 8。
- `OPENAI_ENABLED_TOOLS` / `ENABLED_TOOLS`：默认启用的工具组，支持逗号分隔字符串或 JSON 数组，例如 `file_io,dir_io` 或 `["file_io", "browser_use"]`。
- `OPENAI_INCLUDE_NATIVE_FILE_PARTS` / `INCLUDE_NATIVE_FILE_PARTS`：是否在用户消息中附带原生 `file` 片段。默认 `true`；设为 `false` 时仅附带 `file_id` 文本引用。
...
- `COPAW_BROWSER_USE_DEFAULT`：是否优先使用系统默认浏览器。默认 `true`（即 `1`）；设为 `false` 时强制使用 Playwright 的 Chromium。
...
uv run agent --upload-file ./docs/spec.pdf --user-message "请基于上传文档总结关键需求"
uv run agent --upload-file ./docs/spec.pdf --no-native-file-parts --user-message "仅根据 file_id 引用分析文档"
```
...
- `/files`：查看当前会话已上传文件及其 file_id。
- `/fileparts [on|off]`：查看或切换“原生 file 片段”开关。
- `quit` / `exit` / `退出`：结束对话。
...
3. 后续每条用户消息会自动附带“已上传文件列表”，模型可在回答时引用对应 `file_id`。

说明：当前对话仍通过 Chat Completions 接口执行；当端点不支持消息中的 `file` 片段时，可通过 `OPENAI_INCLUDE_NATIVE_FILE_PARTS=false` 或 `--no-native-file-parts` 关闭原生片段，仅使用 `file_id` 文本引用。
